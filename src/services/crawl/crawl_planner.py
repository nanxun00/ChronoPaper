...
- openreview: keyword search across venues; filtered by openalex_ccf_ranks (CCF A/B/C), no per-venue preference
...
- openalex_ccf_ranks: required when openreview or openalex in sources; controls CCF filtering for both
...
def _validate_plan(raw: dict, existing_sources: list[str] | None) -> dict:
    ...
    if {"openreview", "openalex"} & set(sources) and not oa_ranks:
        oa_ranks = ["A", "B", "C"]
    ...
    if "arxiv" not in sources:
        categories = []
    if "openreview" not in sources and "openalex" not in sources:
        oa_ranks = []
...
def format_plan_summary(plan: dict) -> str:
    ...
    if plan.get("openalex_ccf_ranks"):
        parts.append(f"CCF: {','.join(plan['openalex_ccf_ranks'])}")
...
def build_smart_plan_fields(intent_text: str) -> dict:
    ...
    from src.services.crawl.suggested_paper_verifier import verify_suggested_papers
