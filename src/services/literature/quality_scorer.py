...
def score_openreview_ccf_match(meta: dict[str, Any], task: Any | None) -> float:
    """OpenReview 按 CCF 分级加权（抓取阶段已按 CCF 过滤）。"""
    allowed = {r.upper() for r in _parse_csv_field(getattr(task, "openalex_ccf_ranks", "") or "A,B,C")}
    rank = (meta.get("venue_rank") or "").upper()
    if rank == "A" and "A" in allowed:
        return 95.0
    if rank == "B" and "B" in allowed:
        return 82.0
    if rank == "C" and "C" in allowed:
        return 68.0
    return 50.0
...
def compute_source_preference_score(meta: dict[str, Any], task: Any | None = None) -> float:
    ...
    if source == "openreview":
        return score_openreview_ccf_match(meta, task)
    return 50.0
