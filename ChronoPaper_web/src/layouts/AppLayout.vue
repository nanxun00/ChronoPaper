<script setup>
import { ref, reactive, KeepAlive, onMounted ,watch} from 'vue'
...
const isAdministrator = ref(false); // 是否是管理员
onMounted(() => {
  getRemoteConfig()
  getRemoteDatabase()
  const roleid = sessionStorage.getItem('roleid');
    if (roleid === '1') { // 假设管理员的 roleid 是 1
...
<template>
  <!-- <login v-if="showLoginModal" @close-login="closeLogin" /> -->
  <div class="app-layout" :class="{ 'use-top-bar': layoutSettings.useTopBar }">
    ...
    <div class="header" :class="{ 'top-bar': layoutSettings.useTopBar }">
      ...
      <div class="nav">
        ...
        <RouterLink to="/literature" class="nav-item" active-class="active">
          <component class="icon" :is="route.path.startsWith('/literature') ? ReadFilled : ReadOutlined" />
          <span class="text">文献</span>
        </RouterLink>
        <RouterLink to="/tasks" class="nav-item" active-class="active">
          <component class="icon" :is="route.path.startsWith('/tasks') ? ClockCircleFilled : ClockCircleOutlined" />
      ...
      <RouterLink  class="nav-item personal" to="/personal" active-class="active">
        <component class="icon" :is="route.path === '/personal' ?   UserOutlined : UserOutlined" />
      </RouterLink>
      <RouterLink  class="nav-item setting" to="/setting" active-class="active"  v-if="isAdministrator">
