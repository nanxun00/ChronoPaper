...
# Insert paper figures
fig_candidates = [
    ("figures/Algorithm schematic diagram.png", "DeepDebateCoder框架总体架构图", "图1"),
    ("figures/Denoising process.png", "三层迭代去噪过程示意图", "图2"),
    ("figures/Debate process.png", "非对称辩论工作流示意图", "图3")
]

# Try to find figures from current script directory or SKILL_DIR
import sys
this_script_path = Path(sys.argv[0]).resolve().parent
search_dirs = [
    this_script_path,
    SKILL_DIR,
    SKILL_DIR / "figures",
...
for fig_rel, caption_raw, fig_label in fig_candidates:
    fig_found = None
    for d in search_dirs:
        ...
        if candidate.exists():
            fig_found = candidate
            break
    if fig_found is not None:
        try:
            p = doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            run = p.add_run()
            run.add_picture(str(fig_found), width=Inches(5.5))
            cap = doc.add_paragraph()
            cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
            cap_run = cap.add_run(f"{fig_label}：{caption_raw}")
            cap_run.font.size = Pt(10)
            cap_run.font.name = '宋体'
            cap_run._element.rPr.rFonts.set(qn('w:eastAsia'), '宋体')
        except Exception as e:
            add_paragraph_styled(doc, f"（{fig_label}图片插入失败）", font_size=10)
    else:
        add_paragraph_styled(doc, f"（{fig_label}图片未找到，已跳过）", font_size=10)
...
cap_run = cap_p.add_run('表1：DeepSeek-Chat后端各方法pass@1性能对比（%）')
cap_run.font.size = Pt(10)
cap_run.font.name = '宋体'
cap_run._element.rPr.rFonts.set(qn('w:eastAsia'), '宋体')

# 工业实用性说明
add_heading_styled(doc, '工业实用性', level=2)
add_paragraph_styled(doc, 
    "本发明提供的方法及系统可广泛应用于各类信息系统工程中的代码自动生成与优化场景，"
    "包括但不限于企业级应用开发、微服务架构设计、业务规则引擎实现、数据管道构建等。"
    "通过本发明生成的代码具有结构清晰、异常处理一致、易于维护和扩展等优点，"
    "能够显著提高开发效率、降低维护成本、提升软件质量。")
# Save
output_path = OUTPUT_DIR / "DeepDebateCoder_Patent.docx"
