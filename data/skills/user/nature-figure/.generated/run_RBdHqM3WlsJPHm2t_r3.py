...
# 创建输出目录
output_dir = pathlib.Path("output/runs/RBdHqM3WlsJPHm2t")
output_dir.mkdir(parents=True, exist_ok=True)

# 临时目录设置，避免matplotlib配置目录问题
...
# 保存原始数据
data_file = output_dir / "experiment_data.csv"
...
metrics_to_plot = {
    ...
    'mcc': 'MCC分数',
    'size_mb': '模型大小(MB)',
    'p_res_mae_sec': 'P波MAE(秒)',
    's_res_mae_sec': 'S波MAE(秒)'
}
...
for idx, (metric, title) in enumerate(metrics_to_plot.items()):
    ...
    # 对于MCC分数，处理负值
    if metric == 'mcc':
        colors = ['#d62728' if v < 0 else '#2ca02c' for v in df[metric]]
        ax.bar(range(len(df)), df[metric], color=colors)
    ...
    for i, v in enumerate(df[metric]):
        ax.text(i, v, f'{v:.4f}' if isinstance(v, float) else str(v), 
                ha='center', va='bottom' if v >= 0 else 'top', 
                fontsize=7, color='black')

plt.tight_layout()
plt.suptitle('地震相检测模型性能对比', fontsize=16, fontweight='bold', y=1.02)

# 保存图表
fig.savefig(output_dir / 'model_performance_comparison.png', dpi=300, bbox_inches='tight')
...
comparison_metrics = [
    ('p_f1', 'test_p_f1', 'P波F1分数 (训练 vs 测试)'),
    ('s_f1', 'test_s_f1', 'S波F1分数 (训练 vs 测试)'),
    ('time_acc', 'test_time_acc', '时间准确度 (训练 vs 测试)'),
    ('mcc', 'test_mcc', 'MCC分数 (训练 vs 测试)')
]

for idx, (train_metric, test_metric, title) in enumerate(comparison_metrics):
    if idx >= len(axes2):
        break
    ...
    # 训练指标
    train_bars = ax.bar([i - width/2 for i in x], df[train_metric], width, 
                       label=f'训练', alpha=0.8)
    
    # 测试指标
    test_bars = ax.bar([i + width/2 for i in x], df[test_metric], width, 
                      label=f'测试', alpha=0.8)
    ...
    # 添加差异注释
    for i, (train_val, test_val) in enumerate(zip(df[train_metric], df[test_metric])):
        diff = test_val - train_val
        color = 'red' if diff < 0 else 'green'
        ax.text(i, max(train_val, test_val) + 0.01, f'{diff:.4f}', 
                ha='center', va='bottom', fontsize=6, color=color)

plt.tight_layout()
plt.suptitle('训练与测试指标对比', fontsize=14, fontweight='bold', y=1.02)
fig2.savefig(output_dir / 'train_vs_test_comparison.png', dpi=300, bbox_inches='tight')

# 保存元数据
metadata = {
    "run_id": "RBdHqM3WlsJPHm2t",
    "generated_files": [
        "experiment_data.csv",
        "model_performance_comparison.png",
        "train_vs_test_comparison.png"
    ],
    "metrics_plotted": list(metrics_to_plot.keys()),
    "models_count": len(df)
}
