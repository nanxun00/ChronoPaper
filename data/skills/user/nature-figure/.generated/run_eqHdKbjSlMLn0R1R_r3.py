...
# 在导入任何其他模块之前设置环境变量
os.environ['HOME'] = os.path.expanduser('~') if os.path.expanduser('~') != '~' else os.path.abspath('.')
...
for idx, (metric, label) in enumerate(zip(metrics, metric_labels)):
    ...
    # 绘制柱状图
    bars = ax.bar(range(len(model_names)), values, color=colors, edgecolor='black', linewidth=0.5)
    ...
    for bar, value in zip(bars, values):
        ...
        ax.text(bar.get_x() + bar.get_width()/2., height,
                f'{value:.3f}', ha='center', va='bottom', fontsize=7, rotation=60)
...
# 添加总体标题
fig.suptitle('Model Performance Comparison Across Seeds', fontsize=16, fontweight='bold')
...
test_metrics = ['test_p_f1', 'test_s_f1', 'test_time_acc', 'test_mcc', 'best_val']
test_labels = ['Test P-wave F1', 'Test S-wave F1', 'Test Time Accuracy', 'Test MCC', 'Best Validation']
...
fig2.suptitle('Test Set Performance Comparison', fontsize=16, fontweight='bold')
plt.tight_layout(rect=[0, 0, 1, 0.96])

output_path2 = output_dir / "test_performance.png"
