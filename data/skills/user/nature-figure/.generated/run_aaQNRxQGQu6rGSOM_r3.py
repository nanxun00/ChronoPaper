...
# 设置环境变量以避免路径问题
os.environ['MPLCONFIGDIR'] = os.path.abspath('output/runs/aaQNRxQGQu6rGSOM')
os.environ['HOME'] = os.path.abspath('output/runs/aaQNRxQGQu6rGSOM')
...
# 提取模型名称和关键指标
model_names = [row['name'] for row in rows]
# 选择几个关键指标进行比较：best_val, p_f1, s_f1, test_time_acc, mcc
metrics = {
    'Best Validation': [float(row['best_val']) for row in rows],
    'P-wave F1': [float(row['p_f1']) for row in rows],
    'S-wave F1': [float(row['s_f1']) for row in rows],
    'Test Time Accuracy': [float(row['test_time_acc']) for row in rows],
    'MCC': [float(row['mcc']) for row in rows]
}

# 创建输出目录
output_dir = Path('output/runs/aaQNRxQGQu6rGSOM')
...
for i, (metric_name, values) in enumerate(metrics.items()):
    ax = axes[i]
    bars = ax.bar(model_names, values, color='skyblue', edgecolor='black', alpha=0.8)
    ax.set_title(metric_name, fontsize=12, fontweight='bold')
    ax.set_ylabel(metric_name, fontsize=10)
    ax.set_ylim(min(values) - 0.05 if min(values) >= 0 else min(values) - 0.1, max(values) + 0.05)
    # 旋转x轴标签以避免重叠
...
print(f"已生成柱状图并保存至: {output_dir / 'model_comparison.png'}")
