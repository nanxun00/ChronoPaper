...
# 创建输出目录
output_dir = Path("output/runs/2JLeuQB2uHIb7zWN")
...
test_p_f1_scores = []
test_s_f1_scores = []
test_time_acc_scores = []

for row in rows:
    model_names.append(row['name'])
    test_p_f1_scores.append(float(row['test_p_f1']))
    test_s_f1_scores.append(float(row['test_s_f1']))
    test_time_acc_scores.append(float(row['test_time_acc']))
...
bars1 = ax.bar(x - width, test_p_f1_scores, width, label='test_p_f1', alpha=0.8)
bars2 = ax.bar(x, test_s_f1_scores, width, label='test_s_f1', alpha=0.8)
bars3 = ax.bar(x + width, test_time_acc_scores, width, label='test_time_acc', alpha=0.8)
