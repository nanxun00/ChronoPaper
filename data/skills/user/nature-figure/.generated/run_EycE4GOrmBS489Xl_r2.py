...
import io
import matplotlib
# 设置MPLCONFIGDIR以避免家目录错误
os.environ['MPLCONFIGDIR'] = os.path.abspath('./matplotlib_config')
...
# 提取模型名称和相关指标
model_names = [row[0] for row in data]  # 第一列是name
test_p_f1 = [float(row[headers.index('test_p_f1')]) for row in data]
test_s_f1 = [float(row[headers.index('test_s_f1')]) for row in data]
...
bars1 = plt.bar(x, test_p_f1, width, label='Test P-wave F1', color='#1f77b4')
bars2 = plt.bar([i + width for i in x], test_s_f1, width, label='Test S-wave F1', color='#ff7f0e')
...
plt.title('Model Performance Comparison on Test Set (F1 Scores)', fontsize=14, pad=15)
plt.xticks([i + width/2 for i in x], model_names, rotation=45, ha='right', fontsize=10)
plt.legend(fontsize=11, loc='upper left')
plt.ylim(0, 1.1)  # 设置y轴范围，确保标签可见
plt.tight_layout()

# 保存图表到指定目录
output_dir = pathlib.Path('output/runs/EycE4GOrmBS489Xl')
output_dir.mkdir(parents=True, exist_ok=True)
fig_path = output_dir / 'performance_bar_chart.png'
plt.savefig(fig_path, dpi=300, bbox_inches='tight')
plt.close()

print(f'Bar chart successfully saved to {fig_path}')
