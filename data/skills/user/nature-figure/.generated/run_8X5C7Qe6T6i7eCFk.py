...
# 选择要比较的指标
selected_metrics = ['best_val', 'p_f1', 's_f1', 'test_p_f1', 'test_s_f1']
metric_labels = {
    'best_val': '最佳验证值',
    'p_f1': 'P波F1',
    's_f1': 'S波F1',
...
# 模型名称
model_names = [row['name'] for row in data]
n_models = len(model_names)

# 创建输出目录
run_id = '8X5C7Qe6T6i7eCFk'
...
for i, metric in enumerate(selected_metrics):
    ...
    plt.bar([xi + i * bar_width for xi in x], values, width=bar_width, label=metric_labels[metric])

# 设置图表
plt.xlabel('模型', fontsize=12)
plt.ylabel('指标值', fontsize=12)
plt.title('不同模型性能指标对比', fontsize=14)
plt.xticks([xi + bar_width * (len(selected_metrics) - 1) / 2 for xi in x], model_names, rotation=45, ha='right')
plt.legend()
plt.tight_layout()

# 保存图表
output_file = output_dir / 'model_comparison_chart.png'
